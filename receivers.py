...
class ReceiverManager:
    ...
    def _record_lobs(self, receivers_snapshot, ms):
        ...
        for rx in receivers_snapshot:
            ...
            if not (rx.isActive and rx.doa_time > rx.previous_doa_time and
                    (ms.lob_history_enabled or is_single_rx)):
                continue
    ...
    def _pair_eligible(rx_x, rx_y, ms):
        return (rx_x.confidence >= ms.min_conf and
        ...
        for x, rx_x in rx_snapshot:
            for y, rx_y in rx_snapshot[:x]:
                if not cls._pair_eligible(rx_x, rx_y, ms):
                    continue
                intersection = plot_intersects(rx_x.latitude, rx_x.longitude, rx_x.doa,
                                               rx_y.latitude, rx_y.longitude, rx_y.doa)
                if not intersection:
                    continue
                log.debug("%s", intersection)
                latest_doa_time = max(latest_doa_time, rx_x.doa_time, rx_y.doa_time)
                d2_accum[x].append(v.haversine(rx_x.latitude, rx_x.longitude, *intersection))
                d2_accum[y].append(v.haversine(rx_y.latitude, rx_y.longitude, *intersection))
    ...
    def _single_rx_eligible(rx, ms):
        return (rx.isSingle and rx.isMobile and rx.isActive and
    ...
    def _insert_single_rx_intersection(self, current_time, intersection, conf_a, conf_b):
    ...
    def _triangulate_single_rx(self, rx):
        ...
        for lat_b, lon_b, conf_b, doa_b in lob_array:
            if not self._historical_pair_usable(lat_a, lon_a, doa_a, lat_b, lon_b, doa_b):
                continue
            intersection = plot_intersects(lat_a, lon_a, doa_a, lat_b, lon_b, doa_b)
            if not intersection:
                continue
            if self._insert_single_rx_intersection(current_time, intersection, conf_a, conf_b):
                keep_count += 1
        return keep_count
